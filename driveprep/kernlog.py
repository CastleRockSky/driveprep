...
USB_RESET_PATTERNS = [
    re.compile(r"reset (high|full|low|super)[- ]speed USB device", re.I),
...
def _journal(since: str, until: str | None = None) -> list[str]:
    cmd = ["journalctl", "-k", "--since", since, "--output=short-iso", "--no-pager"]
    if until:
        cmd += ["--until", until]
    try:
...
def _iso_for_journal(stamp: str) -> str:
    """journalctl wants 'YYYY-MM-DD HH:MM:SS', not RFC3339 with a Z."""
...
def sweep(history: ident.LocatorHistory, run_start: str) -> KernelEvents:
    ...
    for epoch in history.epochs:
        ...
        until = _iso_for_journal(epoch.valid_until) if epoch.valid_until else None
