...
**`resume` is not an exception.** A resumed run writes to devices, so it goes through the identical gate: it rebuilds the manifest from the stored state files, re-runs §4.2 eligibility against the live system, recomputes the token from the drives it is about to resume, and requires it. The token will differ from the original batch's token whenever the resumed set is smaller than the original, which is the correct behavior: a drive that finished, was pulled, or was replaced must not be silently swept back into a destructive run. `resume` also re-runs the §4.5 identity sequence per drive and refuses any drive whose identity tuple no longer matches its checkpoint. It holds the same per-drive lock as `run` (§8.1), so it cannot pick up a drive a live run is still erasing.

**A continuing run's match must be unambiguous** (reconnect mid-pass, §9.1, and `resume`). Both write without a fresh confirmation. If more than one attached device matches the identity tuple, refuse; do not take the first. If the tuple's serial is empty — a bridge that exposes none — the tuple is only class + size + model, which an identical drive also satisfies, so additionally require the drive's ATA serial (read through smartctl) to equal the one recorded, and refuse when it differs, cannot be read, or was never recorded.
...
- Some bridges accept `-t` but never update the self-test log. If progress does not change for `no_progress_step_factor` (5) tenths of the estimated duration — drives report progress in 10% steps, so one step is the unit — and never less than 10 minutes, mark the test `inconclusive`, not failed, and say so in the report. The deadline used to be 3x the *whole* estimate, which waited 24.7 hours on a drive wedged at 90% of a 9-hour test.
- **A wedged test is aborted and re-run once** (`extended_stall_retries`). "Wedged" means the drive itself still reports the test in progress at the same percentage; it never logs the test, and waiting achieves nothing. Aborting with `smartctl -X` and starting a fresh test has fixed it every time it has been seen (the re-run finished on schedule). A bridge that reports nothing is not retried, since a second attempt would report nothing either.
...
- **An unfinished drive stays resumable.** It still gets its after-snapshot and an INCOMPLETE report, but building that report advances the checkpoint to the report phase, which `resume` skips; the checkpoint is put back to the phase and offset where the drive stopped. Before this, Ctrl+C at 40% of an erase left a drive `resume` called "nothing to resume".
...
- **Error handling:** an `OSError` carrying a media error at offset X must not abort the pass. That means `EIO` and also `ENODATA`, `EILSEQ`, `ETIMEDOUT` and `EREMOTEIO`: current kernels map an unrecovered read (SCSI sense MEDIUM ERROR) through `BLK_STS_MEDIUM` to `ENODATA`, so handling `EIO` alone ends the verify at a failing drive's first bad sector. Any other errno is a program bug and still stops the pass. Record the byte range, seek past the failing chunk, and continue, so the report can state total bad regions rather than "failed at 1.2 TB." On a media error, optionally re-read the failing chunk at logical-block granularity once, to narrow the bad region from 8 MiB to the actual sectors. The sectors that read back during that re-read must still be checked for zeros, or the rest of the chunk goes unverified; and if every sector reads back, record no unreadable region at all (the kernel log keeps the transient error). Cap recorded ranges at 1000 entries, then summarize, so a totally failed drive does not produce a gigabyte of JSON.
  - `EROFS` / `EACCES` / `EPERM`: fatal, per §4.6.
  - **The same media errors on a write are findings too.** The chunk is recorded as unwritten (`erase.write_error_ranges`) and the fill carries on, so as much of the drive as possible is still overwritten; the drive grades FAIL (§10.2). After `io.max_consecutive_write_failures` (8) failed chunks in a row the erase is abandoned (`erase.write_abandoned`), `erase.performed` is false, and verify and the extended test are skipped: a dead region can take many seconds per failed write, and hours more would not change the verdict. Write media errors used to abort the drive as an unhandled error, grading it INCOMPLETE, which reads as "try again" about a drive that cannot take writes. The report states how far an abandoned erase got and that previous data remains beyond it, and never claims the whole device was overwritten when any region refused the write.
...
- Any region the erase could not write (`erase.write_errors > 0`); that region still holds its previous data
